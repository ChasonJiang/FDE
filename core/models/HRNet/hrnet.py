...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, out_dim:int=59, pool:str="avg",is_official:bool=False):
        self.inplanes = 64
        extra={
            "FINAL_CONV_KERNEL":1,
            "STAGE2":{
        ...
        if is_official:
            ...
            self.head = nn.Sequential(
                ...
                nn.Conv2d(
                    ...
                    stride=2,
                    padding=0),
                nn.AvgPool2d(kernel_size=8,) if pool == "avg" else nn.MaxPool2d(kernel_size=8,),
                nn.Flatten(start_dim=1,end_dim=-1),
                nn.Linear(final_inp_channels,out_dim)
            )
