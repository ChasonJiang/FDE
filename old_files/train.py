...
class Trainer(object):
    ...
    def initConfig(self):
        ...
        self.model_save_path = "./checkpoints/face_net"
    ...
    def init(self):
        ...
        self.model = FDENet(self.num_dim,512)
    ...
    def train(self,):
        ...
        for i in range(self.num_epoch):
            for idx, data in enumerate(self.dataLoader):
                ...
                # loss2=self.lossfunc(output[:,19:-12],labels[:,19:-12])
                ...
                similarity=F.cosine_similarity(output.detach(),labels.detach(),dim=1)
                similarity = torch.mean(similarity)
                distance = F.pairwise_distance(output.detach(),labels.detach(),p=2).mean()
